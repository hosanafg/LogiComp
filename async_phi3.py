...
async def processar_prompts_async(prompt_est, prompt_nat, modelo):
    """Dispara os dois prompts simultaneamente para o Ollama e limita a resposta."""
    # num_predict: 5 impede a LLM de "ficar pensando/escrevendo" além do SAT/UNSAT
    opcoes = {"temperature": 0.0, "num_predict": 5}
    
    tarefa_est = AsyncClient().chat(
        model=modelo,
        messages=[{"role": "user", "content": prompt_est}],
        options=opcoes
    )
    tarefa_nat = AsyncClient().chat(
        model=modelo,
        messages=[{"role": "user", "content": prompt_nat}],
        options=opcoes
    )

    # Executa ambas em paralelo e aguarda o retorno
    resp_est, resp_nat = await asyncio.gather(tarefa_est, tarefa_nat)
    return resp_est, resp_nat


async def analisar_lotes():
    ...
    os.makedirs(pasta_ln, exist_ok=True) 
    ...
    for i_arquivo in range(1, total_arquivos + 1):
        ...
        import time # Importado localmente para garantir o funcionamento do medidor
        ...
        with open(caminho_entrada, mode='r', encoding='utf-8') as f_in:
            ...
            for table_row in leitor:
                ...
                # ---------------------------------------------------
                # PROMPT 1: Abordagem Estruturada
                # ---------------------------------------------------
                ...
                4. Se houver um gargalo de conexões que torne a distribuição IMPOSSÍVEL com apenas 3 frequencies, o modelo é UNSAT.
                ...
                resumos_ln.append([rodada, cenario_narrativo])
                ...
                try:
                    # Cronometra o início do envio para o Ollama
                    inicio_linha = time.time()

                    # Chamada ASSÍNCRONA paralela para os dois caminhos
                    resp_est, resp_nat = await processar_prompts_async(
                        prompt_estruturado, prompt_natural, modelo_llm
                    )

                    # Calcula a duração exata da resposta do modelo
                    tempo_llm = time.time() - inicio_linha
                    ...
                    # Processamento da resposta Natural
                    ...
                    # Agora o print te dá o tempo exato gasto por linha em segundos
                    print(f"--> Linha {rodada:02d} | T:{n:02d} | C:{qtd_conexoes:02d} | Tempo LLM: {tempo_llm:.2f}s | Z3={gabarito_z3} | Tuplas={pred_estruturado} ({status_est}) | Natural={pred_natural} ({status_nat})")
        ...
        # Gravando os resultados fora do loop do leitor CSV
        ...
        with open(caminho_ln, mode='w', newline='', encoding='utf-8') as f_ln:
            escritor_ln = csv.writer(f_ln)
            escritor_ln.writerow(["rodada", "cenario_natural"])
            escritor_ln.writerows(resumos_ln)
            
        taxa_est = (acertos_estruturado * 100.0) / total_lote
        taxa_nat = (acertos_natural * 100.0) / total_lote
        print(f"\nFIM {i_arquivo:02d} | % Acerto LLM Tuplas: {taxa_est:.2f}% | % Acerto LLM Linguagem Natural: {taxa_nat:.2f}%")

    print("\nDEBUG: Todos os testes foram realizados e os cenários textuais foram exportados.")

if __name__ == "__main__":
    # Inicializa o loop de eventos assíncronos do Python
    asyncio.run(analisar_lotes())